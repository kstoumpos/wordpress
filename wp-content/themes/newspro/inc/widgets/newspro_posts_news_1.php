...
class newspro_posts_news_1_widget extends WP_Widget {
    function __construct() {
        $widget_ops = array('classname' => 'newspro_news_block_1', 'description' => __('Posts News block 1', 'newspro'));
        parent::__construct('newspro-posts-news-1', __('Posts News 1', 'newspro'), $widget_ops);
    }
    function widget($args, $instance) {
		...
		while ( $the_query->have_posts() ) : $the_query->the_post(); 
			...
			$date_li='';
			if($show_date == 1) $date_li = '<li class="date">'.get_the_date().'</li>';
			
			$first_news_post .= '
			
   <div class="col-sm-4 col-md-4 newspro_news_large ">
   <a href="' .esc_url(get_permalink()). '">
          <div class="newspro_post_news newspro_large_news" style="background-image: url('.$img_src.')">
				<div class="overlay">
				<div class="newspro_post_data ">
								<span class="p_title">'.$post_title.'</span>								
								<div class="row newspro_post_block_meta">
								<div class="col-sm-6 col-xs-6">'.$comments.' Comments</div>
								<div class="col-sm-6 col-xs-6">'.$date_cat.'</div></div>									
								
							</div><!-- .newspro_post_data -->
							</div>
...
';
	...
	?>
	<div class="row">
		<div class="newspro_posts_block_wrapper">
      <div class="newspro_posts_block newspro_posts_block_3">
         <?php echo $first_news_post; ?>
         
         <div class="clearfix"></div>
      </div> <!-- newspro_posts_block -->
   </div>
   </div>     
     <?php endif; ?>   
        <?php
