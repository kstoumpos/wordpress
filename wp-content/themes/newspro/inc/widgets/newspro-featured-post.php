...
// register newspro_Featured_Post widget
function newspro_register_featured_post_widget() {
    register_widget( 'newspro_Featured_Post' );
}
add_action( 'widgets_init', 'newspro_register_featured_post_widget' );
 
 /**
 * Adds newspro_Featured_Post widget.
 */
class newspro_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'newspro_featured_post', // Base ID
			__( 'newspro: Featured Post', 'newspro' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'newspro' ), ) // Args
		);
	}
...
<?php the_title(); ?>
          ...
          ?>
              ...
              <?php echo get_the_date( 'M j, Y' ); ?>
               <span class="cm1 pull-right"><?php comments_number(); ?></span>
            <?php    
            }
            wp_reset_postdata();
	...
	public function form( $instance ) {
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'newspro' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'newspro' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'newspro' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'newspro' ); ?></label>
		</p>
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'newspro' );
